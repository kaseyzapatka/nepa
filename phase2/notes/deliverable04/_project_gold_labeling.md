...
> **Note (2026-07-15):** this pass is complete; its labels are banked in `training/deliverable04/`.
> The run-once scripts it references (`_emit_project_gold.py`, `apply_project_gold_*.py`,
> `_build_project_gold_parquet.py`) and the `project_gold_options.txt` options view were retired
> after the pass — retrieve them from git history if the pass is ever re-run.
...
`05` aims to follow** (and the definitions in `date_sourcing.md`):
...
**conventions above and `date_sourcing.md` decide**, not the probabilities. Work in chunks of ~50.
