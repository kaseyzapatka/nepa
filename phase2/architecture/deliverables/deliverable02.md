...
| `06_create_figures.R` | primary-scope headline tables + association layer; takes no flags — EIS block auto-runs off the `_eis` parquet (former `--with-eis` removed: it polluted the FONSI tables) | ✅ |
...
Rscript phase2/code/deliverable02/06_create_figures.R                     # EIS block auto-runs; NO --with-eis
quarto render phase2/reports/deliverable02.qmd
```
...
Report: `phase2/reports/deliverable02.qmd`. Figures/tables regenerated by `06_create_figures.R`.
...
`06_create_figures.R` EIS block → `phase2/output/deliverable02/analysis/eis_*.csv`.
