...
output_dir <- here("phase1", "output", "deliverable4")
tables_dir <- here("phase1", "output", "deliverable4", "tables")
figures_dir <- here("phase1", "output", "deliverable4", "figures")
maps_dir <- here("phase1", "output", "deliverable4", "maps")
...
# Optional coagency enrichment (generated by extract_coagency.py)
coagency_join_cols <- c(
  "project_id",
  "dataset_source",
  ...
  "project_coagency_first_hit_page",
  "project_coagency_first_hit_file_name",
  "project_coagency_first_hit_document_type_clean",
  "project_coagency_first_hit_document_type_category",
  "project_coagency_first_hit_is_aux",
  "project_coagency_first_hit_main_document",
  "project_coagency_first_hit_cue",
...
coagency_projects <- tibble(project_id = character(), dataset_source = character())
...
clean_energy_multiagency <- clean_energy %>%
  left_join(coagency_projects, by = c("project_id", "dataset_source")) %>%
  mutate(
    project_has_coagency_signal_any = replace_na(project_has_coagency_signal_any, FALSE),
    project_has_coagency_signal_any_nonaux = replace_na(project_has_coagency_signal_any_nonaux, FALSE),
    project_has_coagency_signal_high_conf = replace_na(project_has_coagency_signal_high_conf, FALSE),
...
cat(
  "Multi-agency (metadata OR coagency high-confidence):",
  sum(clean_energy_multiagency$project_multi_agency, na.rm = TRUE),
  "\n"
)
