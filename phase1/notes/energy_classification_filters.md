...
**Note:** The CSV is generated by running the exploratory section in `phase1/code/deliverable01/02_agency.R`.

---

## Filter 3: Nuclear Waste Management Exclusions

**Location:** `code/extract/extract_data.py` → `apply_energy_type_filters()`
...
**unless** a keep list is present (see override below).
...
**Source (default):** Exclusion terms loaded from `notes/agencies_to_be_excluded.txt`
**Override (keep list present):** If `notes/nuclear_waste_projects_to_keep.csv` exists,
then all Nuclear + Waste Management projects are excluded **except** those in the keep list, which were determined by CAFT to be project relevant.

**Rationale:** Nuclear waste cleanup, storage, and legacy site management are not clean energy production.

**Note:** The keep list represents client-reviewed exceptions and is authoritative
when present. This switches the filter to a **tag-based** rule for Nuclear + Waste,
reclassifying all others to "Other." Clean energy count reduced by ~1,554 (from
~22,279 to ~20,725).
...
**Applies:** Only when the keep list is **not** present (default mode)
...
4. **File dependencies:**
    ...
    - `notes/nuclear_waste_projects_to_keep.csv` - Keep list (client-reviewed)
...
1. Run the exploratory section in `phase1/code/deliverable01/02_agency.R`
