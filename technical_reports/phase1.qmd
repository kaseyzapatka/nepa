...
- The Dataset -> project_overview.html (corpus, classification, exclusions)
...
- Replication Guide -> public repo (README, runbooks, current repository state)
...
This report is a technical companion to the six Phase 1 deliverable reports. It explains how the analysis was built — the dataset, the extraction pipelines, the validation performed — and what someone would need to know to replicate or extend the work. Findings live in the [deliverable reports](../project_overview.html); this document is about the machinery behind them.
...
Phase 1 is complete, and the published state is reproducible from the current repository under `phase1/`. The repository's `phase1/README.md` is the entry point: it maps the directory structure, lists the key facts, and orders the runbooks.
