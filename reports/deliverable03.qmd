...
### How timeline dates are extracted (plain‑language overview)

We start with the full text of each NEPA document and look for **date-like strings** (e.g., “06/07/2018,” “July 7, 2010”). For each date we capture the surrounding sentence so we can understand **why** that date appears. A fast classifier then labels each date into one of three buckets:

- **Initiation**: when the review appears to start (e.g., “application received,” “scoping”)
- **Review**: interim steps (e.g., “phase approved,” “review completed”)
- **Decision**: final approval (e.g., “Approved by… NEPA Compliance Officer”)

**Example phrases by bucket**

- **Initiation**: “Initiator Signature…”, “initiation of consultation…”, “application received…”, “notice of intent…”, “scoping meeting…”
- **Review**: “Phase 1 was approved…”, “review completed…”, “interim review…”, “concurrence provided…”, “decision in principle…”
- **Decision**: “Approved by [Name], NEPA Compliance Officer…”, “Authorizing Official…”, “Field Office Manager determination…”, “digitally signed by…”, “FONSI signed…”

When no explicit initiation date is found, we use the **earliest review date** as a proxy for initiation. To reduce false positives, we **strip boilerplate lines** (e.g., standard form text) before classification, while still keeping the original text for verification. This creates a transparent, auditable timeline: every date can be traced back to the exact text it came from.
...
### Example Project Timelines (BERT v8 Classifier)

The tables below show three example projects and every date the classifier extracted from their documents. Each date is labeled as **initiation**, **review**, **decision**, or **other**, along with the source text that triggered the classification. These examples illustrate how the classifier constructs a project timeline from raw document text.

```{r}
#| label: tbl-example5
#| tbl-cap: "Example 5: Extracted timeline"
...
```{r}
#| label: tbl-example3
#| tbl-cap: "Example 3: Extracted timeline"
...
```{r}
#| label: tbl-example1
#| tbl-cap: "Example 1: Extracted timeline"
